...
class PriceDirective(TypedDict):
    currency: str
    amount: str
    base_currency: str
...
class AvailabilityDay:
    d: str
    n: int
    directives: list[PriceDirective]
...
def _format_price_directive(p: Price) -> PriceDirective:
    return {
        "currency": p.currency,
        "amount": str(p.amount.number),
        "base_currency": p.amount.currency,
        "generated_by": p.meta.get("generated_by") if p.meta else None,
        "filename": p.meta.get("filename") if p.meta else None,
        "lineno": p.meta.get("lineno") if p.meta else None,
    }
